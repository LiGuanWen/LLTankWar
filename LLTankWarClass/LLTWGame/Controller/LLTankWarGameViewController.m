...
@interface LLTankWarGameViewController ()
@property (strong, nonatomic) UIView *mainView;

@property (strong, nonatomic) UIView *controlView;

@property (strong, nonatomic) LLTWTank  *mainTank;
...
@property (strong, nonatomic) LLTWFactory *factory;
...
@implementation LLTankWarGameViewController


- (void)dealloc {
    NSLog(@"LLTankWarGameViewController 内存释放");
}
...
- (void)restart {
    [LLTWFactory clearAll];
    [LLTWFactory setIsGameover:false];
    _mainTank.life = 3;
    stage = -1;
    stageCount = 1;
    _mainTank.level = 0;
...
- (void)initUI {
    ...
    LLTWXibUtil *util = [[LLTWXibUtil alloc] initWithMainView:self.mainView];
    for (LLTWWall *w in [util getWallsFromXib:@"Map0"]) {
...
- (void)btnClick:(UIButton *)sender {
    if (fireTag != sender.tag && !_playBtn.selected && ![LLTWFactory isGameover]) {//方向键
        self.mainTank.canMove = true;
        //sender.tag - baseTag就是方向的枚举
        _mainTank.dir = sender.tag - baseTag;
        
    } else {//fire
        for (LLTWBullet *b in _mainTank.bullets) {
            if (!b.canMove) {
                b.canMove = true;
                break;
            }
...
- (void)refresh:(NSTimer *)timer {
    ...
    if (_mainTank.life > 0 && _mainTank.isDeath) {
        if (_mainTank.life > 0) {
            ...
            _mainTank.life = life;
            _info.text = [NSString stringWithFormat:@"我方：%zd", _mainTank.life];
            [self zjgh];
        }
    }
    if (_mainTank.life <= 0 && _mainTank.isDeath) {
    ...
    for (LLTWBullet *b in self.mainTank.bullets) {
...
- (void)pass {
    ...
    if (count_tank >= badTankCount && [LLTWFactory instanceOfBadTankArray].count == 0 && ![LLTWFactory isBorining] && ![LLTWFactory isGameover]) {
...
- (void)excessive:(NSString *)tip {
    LLTWGameOverViewController *pv = [LLTWGameOverViewController new];
    pv.tip = tip;
    _timer.fireDate = [NSDate distantFuture];
    pv.restartGameBlock = ^{
        [self restart];
...
//    }
    _timer.fireDate = [NSDate distantPast];
    [self playingSoundEffectWith:@"lltankwar_start.wav"];
    
    if (![LLTWFactory isBorining] && ![LLTWFactory isGameover]) {
        stage++;
...
#pragma mark - 开挂
        ...
        LLTWXibUtil *util = [[LLTWXibUtil alloc] initWithMainView:self.mainView];
        for (LLTWWall *w in [util changeMap:[NSString stringWithFormat:@"Map%d", stage]]) {
            [self.mainView addSubview:w.imgView];
            [self.mainView bringSubviewToFront:w.imgView];
        }
    }
