...
//全是子弹
static NSMutableArray<LLTWRectangle *> *needCheckKnocked;
static NSMutableArray<LLTWRectangle *> *badTankArray;
...
- (LLTWTank *)createTank:(TankType)tankType direction:(TankDirectione)dir center:(CGPoint)center {
    ...
    LLTWBullet *b = [self createBullet:dir isGood:tankType == TankTypeMain];
...
- (LLTWBullet *)createBullet:(TankDirectione)dir isGood:(BOOL)isGood {
    ...
    //用户与碰撞检测
    [[LLTWFactory instanceOfNeedCheckKnocked] addObject:bu];
    return bu;
}
...
#pragma mark - 测试
    ...
    //加入敌方坦克数组
    [[LLTWFactory instanceOfBadTankArray] addObject:bad];
    
    isBorning = false;
}

- (BOOL)canBorn:(UIImageView *)view {
    for (LLTWRectangle *r in badTankArray) {
        //检测到碰撞
...
+ (NSMutableArray<LLTWRectangle *> *)checkKnockWidthTank:(LLTWRectangle *)r1 {
    ...
    for (LLTWRectangle *r in needCheckKnocked) {
        //检测到碰撞
        if (r1 != r && [LLTWFactory checkIsKnockedWithRect:r1.imgView r2:r.imgView]) {
            [arr addObject:r];
        }
    }
    return arr;
...
+ (NSMutableArray<LLTWRectangle *> *)checkKnockWidthWall:(LLTWRectangle *)r1 {
    NSMutableArray<LLTWRectangle *> *arr = [NSMutableArray array];
    for (LLTWWall *r in [LLTWXibUtil getAllWalls]) {
        //检测到碰撞
        if (r1 != r && [LLTWFactory checkIsKnockedWithRect:r1.imgView r2:r.imgView]) {
...
+ (LLTWRectangle *)checkIsKnockedWithRectArray:(LLTWRectangle *)r1 {
    for (LLTWRectangle *r in badTankArray) {
        //检测到碰撞
        if (!r.isDeath && r1.isGood != r.isGood && r1 != r && [LLTWFactory checkIsKnockedWithRect:r1.imgView r2:r.imgView]) {
            return r;
        }
    }
    for (LLTWRectangle *r in needCheckKnocked) {
